...
// ---------- Poster tree (export-only) ----------
// Rendered off-screen at a fixed wide width with classic connecting
// lines. Deliberately NOT the same markup as the on-screen page —
// the on-screen version must stay responsive and never overflow on
// any device, while this one only ever needs to look good as a single
// static image, so it can afford a fixed desktop-poster layout.
function PosterPersonCard({ node }: { node: Person }) {
  const accent = tierAccent(node.role);
  return (
...
function PosterDeptBox({ dept }: { dept: DeptNode }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      <div style={{ width: 2, height: 22, background: "#cbd5e1" }} />
      <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e2e8f0", borderTop: "4px solid #092F69", padding: "14px 16px", width: 300 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          <BuildingIcon size={18} />
          <div>
            <div style={{ fontWeight: 800, fontSize: 14.5 }}>{dept.name}</div>
            <div style={{ fontSize: 11, color: "#64748b" }}>{dept.headcount} people{dept.headName ? ` · Head: ${dept.headName}` : ""}</div>
          </div>
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
          {dept.directMembers.map((p) => <PosterPersonCard key={p.id} node={p} />)}
        </div>
        {dept.teams.map((team) => (
          <div key={team.id} style={{ marginTop: 10, paddingTop: 10, borderTop: "1px dashed #cbd5e1" }}>
            <div style={{ fontSize: 11.5, fontWeight: 700, color: "#b45309", marginBottom: 6 }}>{team.name} · {team.members.length}</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
              {team.members.map((p) => <PosterPersonCard key={p.id} node={p} />)}
            </div>
          </div>
        ))}
      </div>
...
function OrgChartPoster({ companyName, leadership, tree, totalPeople }: { companyName: string; leadership: Person[]; tree: DeptNode[]; totalPeople: number }) {
  return (
    <div style={{ width: 1400, padding: 32, background: "#f8fafc", fontFamily: "Georgia, serif", boxSizing: "border-box" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, padding: "18px 24px", borderRadius: 14, background: "linear-gradient(135deg, #092F69, #05173a)", color: "#fff", marginBottom: 4 }}>
        <BuildingIcon size={24} />
        <div>
          <div style={{ fontWeight: 800, fontSize: 20 }}>{companyName}</div>
          <div style={{ fontSize: 12, opacity: 0.85 }}>{totalPeople} people across {tree.length} departments</div>
        </div>
      </div>

      {leadership.length > 0 && (
        <>
          <div style={{ display: "flex", justifyContent: "center" }}><div style={{ width: 2, height: 22, background: "#cbd5e1" }} /></div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, justifyContent: "center", marginBottom: 4 }}>
            {leadership.map((p) => <PosterPersonCard key={p.id} node={p} />)}
          </div>
        </>
      )}
      ...
      {tree.length > 1 && (
        <div style={{ display: "flex", justifyContent: "center" }}>
          <div style={{ height: 2, background: "#cbd5e1", width: `${Math.min(tree.length * 320, 1300)}px` }} />
        </div>
      )}
      <div style={{ display: "flex", gap: 24, justifyContent: "center", flexWrap: "wrap", alignItems: "flex-start" }}>
        {tree.map((dept) => <PosterDeptBox key={dept.id} dept={dept} />)}
      </div>

      <div style={{ textAlign: "center", marginTop: 24, fontSize: 11, color: "#94a3b8" }}>
        Generated by ORBIT-I Pulse · {new Date().toLocaleDateString("en-PK", { year: "numeric", month: "long", day: "numeric" })}
      </div>
    </div>
  );
}
...
export default function OrgChartPage() {
  ...
  async function downloadChart() {
    ...
    try {
      ...
      const dataUrl = await toPng(posterRef.current, { backgroundColor: "#f8fafc", pixelRatio: 2, cacheBust: true });
  ...
  return (
    <main className="dash-content fade-up">
      ...
      {/* Off-screen poster used ONLY for the "Download as image" export —
          fixed-width connector-line tree, never shown to the user, never
          affects the responsive on-screen layout above. */}
      {canDownload && tree.length > 0 && (
        <div style={{ position: "fixed", top: 0, left: -99999, pointerEvents: "none" }} aria-hidden="true">
          <div ref={posterRef}>
            <OrgChartPoster companyName={companyName} leadership={leadership} tree={tree} totalPeople={totalPeople} />
          </div>
        </div>
      )}
    </main>
